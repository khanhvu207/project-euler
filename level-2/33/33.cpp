#include <bits/stdc++.h>
using namespace std;
#define fi first
#define se second
#define ll long long
#define mp make_pair
#define pb push_back
#define eb emplace_back
#define sz(a) int((a).size())
#define all(a) (a).begin(), (a).end()
#define TLE() cerr<<"\nTime elapsed: "<<1000*clock()/CLOCKS_PER_SEC<<"ms.\n";
const ll MOD = 1E9+7;
const ll IINF = 1E18;
const int INF = 1e9;
const double EPS = 1E-9;
const double PI = atan(1.0) * 4.0;
template<class T> T getbit(T s, int i) {return s>>i&1;}
template<class T> void flipbit(T& s, int i) {s^=(1LL<<i);}
constexpr int cntbit(ll x) {return __builtin_popcountll(x);}
constexpr int lg2(ll x) {return __lg(x);} // return int(log2(x))
// mt19937_64 rng(chrono::steady_clock::now().time_since_epoch().count());
mt19937 rng((uint32_t)chrono::steady_clock::now().time_since_epoch().count()); 
ll fpow(ll a,ll p){ll res=1LL;while(p){if(p&1LL) res=res*a%MOD;a=a*a%MOD;p>>=1LL;} return res;}

void io_optimization() {
	ios_base::sync_with_stdio(0); cin.tie(0);
}

int main() {
	int resx = 1;
	int resy = 1;
	for (int i = 10; i <= 99; ++i) {
		for (int j = i + 1; j <= 99; ++j) {
			int maski = 0;
			int maskj = 0;
			int x = i;
			while (x > 0) maski |= (1 << (x % 10)), x /= 10;
			x = j;
			while (x > 0) maskj |= (1 << (x % 10)), x /= 10;
			if (cntbit(maski & maskj) != 1) continue;
			int joint = maski & maskj;
			int d = 0;
			for (; d < 10; ++d) 
				if ((joint ^ (1 << d)) == 0) break;
			if (d == 0) continue;
			int newi = i;
			int newj = j;
			if (d == newi % 10) newi /= 10;
			else newi %= 10;
			if (d == newj % 10) newj /= 10;
			else newj %= 10;
			if (newj == 0) continue;
			// cerr << i << ' ' << j << ' ' << newi << ' ' << newj << '\n';
			int gcd = __gcd(i, j);
			int _i = i / gcd;
			int _j = j / gcd;
			int _gcd = __gcd(newi, newj);
			int _newi = newi / _gcd;
			int _newj = newj / _gcd;
			if (_i == _newi && _j == _newj) {
				// cerr << i << ' ' << j << ' ' << newi << ' ' << newj << '\n';
				resx *= i;
				resy *= j;
			}
		}
	}
	int gcd = __gcd(resx, resy);
	cout << resy / gcd << '\n';
	// ^ solution here
	TLE();
	return 0;
}